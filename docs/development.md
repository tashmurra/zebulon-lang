...
The macOS linker is selected from `ZEB_LD64_LLD` (an executable path or name), then `ld64.lld` in `llvm-config --bindir`, then `ld64.lld` on `PATH` if the bundled linker is absent. An invalid explicit setting or an invalid bundled linker is an error, not a reason to fall back. The selected linker must report LLVM's pinned version, 22.1.8, and its path and contents are included in `native-tool-lock.json` and build/cache identities. Full LTO uses this resolved executable.
